...
class ImageFilter:
    ...
    def add_convolution_result(self, matrix, kernel_1, kernel_2, hence, filtername):
        ...
        if hence is True:
            ...
            filtername += '_hence'
        # 打印图片
        self.put_image(new_l_str, filtername)
        self.exe_time(start, end, filtername)
    ...
    def canny(self, high=150, low=50, sigma=1.4, size=3, expand='zero'):
        ...
        # 打印图片
        self.put_image(new_l_str, filter_name)
        self.exe_time(start, end, filter_name)
        return new_l_matrix
    ...
    def avg_filter(self, mode='zero', size=3):
        ...
        # 打印图片
        self.put_image(new_l_str, filter='avg')
        self.exe_time(start, end, filter='avg')
        return new_l_matrix
    ...
    def medial_filter(self, mode='zero', size=3):
        ...
        # 打印照片
        self.put_image(new_l_str, filter='medial')
        self.exe_time(start, end, filter='medial')
        return new_l_matrix

    # 打印运行信息
    # 包括：文件名、文件大小、卷积核大小、边界扩张方式、滤镜名、运行时间
    def exe_time(self, start, end, filter):
        s_file = 'File: ' + self.filename + '\n'
        s_size = 'Size: ' + str(self.xsize) + '*' + str(self.ysize) + '\n'
        s_coresize = 'Core size: ' + str(self.core_size) + '*' + str(self.core_size) + '\n'
        s_expand_mode = 'Expand mode: by ' + self.expand_mode + '\n'
        s_filter = 'Fileter: ' + filter + '\n'
        s_time = "Execute time: " + str(round(end - start, 3)) + 's' + '\n'
        print(s_file + s_size + s_coresize + s_expand_mode + s_filter + s_time)
    ...
    def put_image(self, data, filter):
        ...
        s = str(self.core_size) + '*' + str(self.core_size)
        self.im.save(filter + '_' + self.expand_mode + '_' + self.get_filename() + '_' + s + '.' + self.get_format())
        # 恢复原图数据
        self.im.putdata(self.l_str)
...
# y.canny(sigma=1.5, low=50, high=100)
# y.sobel()
# y.gauss()
